...
public class QuickAccessPage : FmgLibContentPage
{
    ...
    public override void Build()
    {
                ...
                .Content(
                    new VerticalStackLayout()
                        .Children(
                            ...
                            new Label()
                                .Text("Sık kullanılan sayfalara kısayol")
                                .TextColor(Colors.White)
                                .Opacity(0.9)
                                .HorizontalTextAlignment(TextAlignment.Center)
                        )
        ...
        // Kutu üreten yardımcı
        Frame Tile(string title, string emoji, string subtitle, Func<Task> onTap)
        {
            var card =
                new Frame()
                    .CornerRadius(16)
            ...
            var v =
                new VerticalStackLayout()
                    .Spacing(6)
                    .Children(
                        new Label().Text($"{emoji}").FontSize(22).TextColor(Colors.White),
                        new Label().Text(title).FontAttributes(FontAttributes.Bold).FontSize(16).TextColor(Colors.White),
                        new Label().Text(subtitle).FontSize(12).Opacity(0.9).TextColor(Colors.White)
                    );
        ...
        // Satır/sütun ekleme kolaylığı
        void Add(View view, int col, int row)
        {
        ...
        Add(Tile("Müşteri Yönetimi", "👤", "Liste • Ara • Ekle", async () => await Navigation.PushAsync(new CustomersPage())), 0, 0);
        Add(Tile("Konaklama", "🏠", "Okul yurtları & daireler", async () => await Navigation.PushAsync(new AccommodationsPage())), 1, 0);
        Add(Tile("Demirbaş", "🪑", "Varlık envanteri", async () => await Navigation.PushAsync(new AssetsPage())), 0, 1);
        Add(Tile("Kiralama", "📦", "Sözleşme & süre yönetimi", async () => await Navigation.PushAsync(new LeasePage())), 1, 1);
        Add(Tile("Ödemeler", "💳", "Tahsilat & ödeme planı", async () => await Navigation.PushAsync(new PaymentsPage())), 0, 2);
        Add(Tile("Muhasebe", "📊", "Genel bakış & raporlar", async () => await Navigation.PushAsync(new AccountingPage())), 1, 2);
        ...
        var root =
            new VerticalStackLayout()
                ...
                .Children(
                    ...
                    grid,
                    new Label()
                        .Text("İpucu: Ana sayfadaki kırmızı banttan da ulaşabilirsiniz.")
                        .FontSize(12)
...
//                               .TextColor(Colors.White)
//                               .HorizontalTextAlignment(TextAlignment.Center),
//                    new Label().Text("Genel Bakış • Hesaplar • Yevmiye • Raporlar")
...
//        var b1 = TabBtn("Genel Bakış", () => ShowDashboard = true);
...
//        // Para Hesapları mini liste
//        var lbl = new Label { Text = "Para Hesapları", FontAttributes = FontAttributes.Bold, Margin = new Thickness(0, 4, 0, 0) };
...
//        var search = new Entry { Placeholder = "Açıklama / hesap ara...", BackgroundColor = LightGray, HeightRequest = 44 };
...
//        var title = new Label { Text = "Aylık Gelir - Gider Özeti", FontAttributes = FontAttributes.Bold, FontSize = 18 };
...
//        var amtStr = await DisplayPromptAsync(kind, "Tutar giriniz:", "Kaydet", "İptal", keyboard: Keyboard.Numeric);
...
//        var desc = await DisplayPromptAsync(kind, "Açıklama (isteğe bağlı):", "Devam", "Atla") ?? "";
...
//            Description = string.IsNullOrWhiteSpace(desc) ? $"{kind} kaydı" : desc,
//            Account = acc.Name,
...
//        Journal.Add(new JournalRow { Date = t.AddDays(-4), Kind = "Gelir", Category = "Kira", Description = "Öğrenci kira tahsilatı", Account = "Banka JPY", Currency = "JPY", Amount = 140000 });
...
//        Journal.Add(new JournalRow { Date = t.AddDays(-2), Kind = "Gelir", Category = "Okul", Description = "Kayıt ücreti", Account = "Kasa TRY", Currency = "TRY", Amount = 12000 });
//        Journal.Add(new JournalRow { Date = t.AddDays(-1), Kind = "Gider", Category = "Ofis", Description = "Elektrik faturası", Account = "Banka TRY", Currency = "TRY", Amount = 3500 });
//        Journal.Add(new JournalRow { Date = t, Kind = "Gelir", Category = "Kira", Description = "Kira tahsilatı", Account = "Banka JPY", Currency = "JPY", Amount = 160000 });
...
//        public string Type { get; set; } = "Kasa"; // Kasa | Banka | Borç | Alacak | Diğer
